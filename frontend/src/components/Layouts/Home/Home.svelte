  ...
  <div class="[ home-grid ] [ border-radius-cubed  gap-2 ]" data-grid-collapse>
    {#if showFilters || $mq.state !== 2}
      ...
      >
        <Button
          ...
          cubeClass={{
            utilClass:
              "flex gap-1 margin-block-end-1 width-100 justify-content-center",
          }}
        >
          <p>Search</p>
        ...
        <ListSelect
          bind:selected={selectedFilterHook}
    ...
    <div class="[ product__results ]">
          ...
          <ul role="list" class="[ flex gap-1 flex-wrap ]">
            {#if $selectedFilterHook.length > 0}
              {#each $selectedFilterHook as item}
                <li>
                  <Button
                    variant="filter"
                    on:click={() => selectedFilterHook.remove(item)}
                  >
                    <FlexyCustom>
                      <p>{item}</p>
                      <Icon cubeClass={{ utilClass: "fs-350" }}
                        >{TIMES_ICON}</Icon
                      >
                    </FlexyCustom>
                  </Button>
            ...
            {:else}
              <li>
                <Button
                  variant="filter"
                  cubeClass={{ utilClass: "ignore-self" }}>No categories</Button
                >
              </li>
            {/if}
          </ul>
      ...
      <section class="[ margin-block-start-1 ]">
        ...
        <div
          class="[ products ] [ grid gap-2 margin-block-start-2 ]"
          data-mode={gridLayout}
        >
          <ComponentMap
...
{:catch}
  <div class="[ grid place-items-center ]">
    <Card
      variant="primary"
      cubeClass={{ utilClass: "padding-block-1 padding-inline-2" }}
    >
      <FlexyCustom>
        <Icon ariaLabel="Info icon" cubeClass={{ utilClass: "clr-primary-400" }}
          >{INFO_ICON}</Icon
        >
